...
"${GN_BIN}" gen "${OUT_DIR}" --args='target_os="linux" is_debug=false is_clang=false rtc_include_opus=true rtc_include_tests=true rtc_build_examples=true rtc_build_sdk=false rtc_enable_symbol_export=true rtc_use_speech_audio_devices=true use_custom_libcxx=false enable_js_protobuf=false'
