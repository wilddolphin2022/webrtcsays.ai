...
"${GN_BIN}" gen "${OUT_DIR}" --args='target_os="linux" is_debug=false is_clang=false use_sysroot=false treat_warnings_as_errors=false rtc_include_opus=true rtc_include_tests=true rtc_build_examples=true rtc_enable_symbol_export=true rtc_use_speech_audio_devices=true use_custom_libcxx=false enable_js_protobuf=false rtc_enable_protobuf=false enable_libaom=false'
