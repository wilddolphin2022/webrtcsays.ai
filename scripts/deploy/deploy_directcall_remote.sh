...
LLAMA_9B_FILE="Qwen3.5-9B-Q3_K_M.gguf"
...
if [ "${DEPLOY_USER}" != "root" ]; then
  ssh "${SSH_OPTS[@]}" "${REMOTE}" "sudo mkdir -p '${DEPLOY_PATH}' /opt/whillats /opt/models && sudo chown -R '${DEPLOY_USER}:${DEPLOY_USER}' '${DEPLOY_PATH}' /opt/whillats /opt/models"
fi
...
Restart=always
RestartSec=2
Environment=LD_LIBRARY_PATH=${DEPLOY_PATH}/lib:/usr/local/cuda/lib64:/usr/local/cuda/lib64
...
Environment=GGML_BACKEND_DL_SEARCH_PATH=${DEPLOY_PATH}/lib
